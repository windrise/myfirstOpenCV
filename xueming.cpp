	...
	const bool&						normflag)
{
	...
	{
		std::vector<double>::iterator Lbiggest = std::max_element(std::begin(slvec),std::end(slvec));
		std::vector<double>::iterator Lsmall= std::min_element(std::begin(slvec), std::end(slvec));

		std::vector<double>::iterator Abiggest = std::max_element(std::begin(savec), std::end(savec));
		std::vector<double>::iterator Asmall = std::min_element(std::begin(savec), std::end(savec));

		std::vector<double>::iterator Bbiggest = std::max_element(std::begin(sbvec), std::end(sbvec));
		std::vector<double>::iterator Bsmall = std::min_element(std::begin(sbvec), std::end(sbvec));
		
		for (int i = 0; i < sz; i++)
		{
			slvec[i] = (slvec[i] - avgl)*(slvec[i] - avgl);
			savec[i] = (savec[i] - avga)*(savec[i] - avga);
			sbvec[i] = (sbvec[i] - avgb)*(sbvec[i] - avgb);

		}

		for (int i = 0; i < sz; i++)
		{
//			slvec[i] = (slvec[i] - *Lsmall) / (*Lbiggest-*Lsmall);
			savec[i] = (savec[i] - *Asmall) / (*Abiggest - *Asmall);
			sbvec[i] = (sbvec[i] - *Bsmall) / (*Bbiggest - *Bsmall);
		}
		...
		for (int i = 0; i < sz; i++)
		{
			salmap[i] = slvec[i] + (*Abiggest - *Asmall)*savec[i] + (*Bbiggest - *Bsmall)*sbvec[i];
